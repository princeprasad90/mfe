...
const toContainer = (
  ...
  const container = (
    maybeContainer?.get ? maybeContainer : maybeContainer?.default
  ) as Partial<FederatedContainer> | undefined;
  if (!container?.get) {
    throw new Error(
...
const getContainer = async (
  ...
  // Accept both /assets/remoteEntry.js (production build) and /remoteEntry.js (dev mode)
  if (!remoteEntry.includes("remoteEntry.js")) {
    throw new Error(
      `Invalid remoteEntry for ${remoteEntry}. MFEs must expose remoteEntry.js.`,
    );
  }
...
export const loadRemoteVite = async <TModule = any>(
  remoteEntry: string,
  module: string,
): Promise<TModule> => {
